<?php
/**
 * The table used to display the login history
 *
 * @since   1.0.0
 * 
 ...
 */
class Login_History_Table_Auth_Log extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        $perPage = $this->get_items_per_page('login_history_entries_per_page', LOGIN_HISTORY_DEFAULT_ENTRIES_PER_PAGE);
	...
	public static function conditional_sql() {
		...
		if ( isset( $_GET['s'] ) and ! empty( $_GET['s'] ) ) {
            ...
            $where[] = "`ip_address` LIKE '%{$search}%' OR `username` = '{$search}'";
		}
		...
		if ( isset( $_GET['time_period'] ) and ! empty( $_GET['time_period'] ) ) {
            ...
            switch ($time_period_selected) {
                ...
                case '24hrs': 
                    $time_to_select = time() - LOGIN_HISTORY_SECONDS_IN_1_DAY;
                    break;

                case '7days':
                    $time_to_select = time() - 7*LOGIN_HISTORY_SECONDS_IN_1_DAY;
                    break;

                case '30days':
                    $time_to_select = time() - 30*LOGIN_HISTORY_SECONDS_IN_1_DAY;
                    break;
            }
			if ($time_to_select)
		...
		//Check login result code. A successful login has a code of 'success'.
     ...
     * @return  Array
     */
    private function table_data($per_page = LOGIN_HISTORY_DEFAULT_ITEMS_PER_PAGE, $page_number = 1)
    {
    ...
    private function count_entries_by_date() {
        ...
        $time_24hrs_ago = time() - LOGIN_HISTORY_SECONDS_IN_1_DAY;
        $time_7days_ago = time() - (7 * LOGIN_HISTORY_SECONDS_IN_1_DAY);
        $time_30days_ago = time() - (30 * LOGIN_HISTORY_SECONDS_IN_1_DAY);
    ...
    /**
	 * Displays the search box, enabling search by IP address or username
	 *
	 * @since 1.0.0
	 *
    ...
    public function result_code_dropdown( ) {
		...
		$options = array (
			'0' => __( 'All logins', 'login-history' ),
			'success' => __( 'Successful logins', 'login-history' ),
			'failed' => __( 'Failed logins', 'login-history' ),			
		);
		...
		?><label class="screen-reader-text" for="result_code"><?php
			_e( 'Filter by result type', 'login-history' ); 
		?></label>
    ...
    public function get_views() {
		...
		$views['all'] = "<a href='{$all_url }' {$class} >" . __( "All", 'login-history' ) . " <span class=\"count\">(" . $num_posts['all'] . ")</span></a>";
		$views_item   = array(
			'24hrs'   => array( "name" => __( "Last 24 hours", 'login-history' ), "status_id" => 1 ),
			'7days' => array( "name" => __( "Last 7 days", 'login-history' ), "status_id" => 0 ),
			'30days'    => array( "name" => __( "Last 30 days", 'login-history' ), "status_id" => 2 )
		);
	...
	public function column_ip_address( $item ) {
		...
		$delete_nonce = wp_create_nonce( 'lh_delete_log_entry' );
        ...
        if ($item['ip_address'])
            return esc_html($item['ip_address']) . $this->row_actions( $actions );
        else
            return esc_html( __('Unknown','login_history')) . $this->row_actions( $actions );
    ...
    public function column_default( $item, $column_name )
    {
        switch( $column_name ) {
            ...
            case 'result_description':
                $truncated_error_message = Login_History::get_first_sentence($item[ $column_name ]);
    ...
    {
        $actions = array(
            'bulk-delete' => __('Delete','login-history')
            );
    ...
    protected function process_bulk_action()
    {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'lh_delete_log_entry' ) ) {
				return;
			}
			else {
    ...
    public function get_columns()
    {
        $columns = array(
            ...
            'attempt_time'    => __('Time','login-history'),
            'ip_address'       => __('IP Address','login-history'),
            'ip_location'   => __('IP Location','login-history'),
            'device_type'   => __('Device Type','login-history'),
            'username' => __('Username','login-history'),
            'result_code'    => __('Result','login-history'),
            'result_description'      => __('Message','login-history')
        );
    ...
    public function no_items() {
        if ( (isset($_GET['time_period'])) || (isset($_GET['s'])) || (isset($_GET['result_type']))  ) 
            _e( 'No records found.','login-history' );
        else
            _e( 'Login history is now active. All future Login attempts will appear in this table.','login-history' );
    }
